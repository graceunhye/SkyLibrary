					...
					<div class="content">
						<div class="searchview">
								...
								<table width="800px" class="viewTable" >
									<tr height="80px;">
										<td colspan="3" align="center" height="50" class="view_td">
											<h2>&lt;&lt;&nbsp;&nbsp; <c:out value="${bestView.bookSubject}" /> &nbsp;&nbsp;&gt;&gt;</h2>
										</td>
									</tr>
									<tr class="tr">
										<td rowspan="6" width="15%" align="center" class="imgtd">
											<img src="<c:out value="${bestView.bookCoverImg}" />" alt="<c:out value="${bestView.bookSubject}" />" width="82px">
										</td>
										<th>저자</th>
										<td><c:out value="${bestView.bookWriter}" /></td>
									</tr>
									<tr class="tr">
										<th>출판사</th>
										<td><c:out value="${bestView.bookCompany}" /></td>							
									</tr>
									<tr class="tr">
										<th>발행일</th>
										<td><c:out value="${bestView.bookPublicationDate}" /></td>							
									</tr>
									<tr class="tr">
										<th>ISBN</th>
										<td><c:out value="${bestView.bookISBN}"/></td>							
									</tr>
									<tr class="tr lasttd">
										<th>소장기관</th>
										<td>하늘 도서관</td>							
							...
							<h3><font color="orange">*</font>상세정보</h3>
								<div class="storyDiv">
									<c:out value="${bestView.bookStory}"/>
								</div>
							</div>
							<br>
							...
							<form><input type="hidden" ></form>
								<table class="viewTable" width="800">
									...
									<c:choose>
										<c:when test="${bestView.bookRent == 0}" >
										<tr height="40px">
											...
											<td class="lasttd" align="center" id="rent3">
												<c:choose>							
													...
													<c:when test="${user.userID != null}">
														<input type="button" value="대출하기" class="rentbtn" onclick="rentFn('<c:out value="${bestview.bookISBN}"/>'), <c:out value="${cnt}" />">
													</c:when>
												</c:choose>	
											</td>
										</tr>
										</c:when> 
										<c:when test="${bestView.bookRent == 1}">
											<tr height="40px">
												<td class="lasttd" align="center">대출불가(대출중)</td>
												<td class="lasttd" align="center"><c:out value="${bestview.rentEndDate}"/></td>
